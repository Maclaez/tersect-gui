...
$(document).ready(() => {
  ...
  $.post('/', {
    ...
    for (i = 0; i < samples.length; i++) {
      $('<td>' + samples[i] + '</td>').attr({
        id: [i],
        class: 'samples'
      }).appendTo('#genomeTable').draggable({
        opacity: 0.5,
        helper: "clone",
  ...
  $("#searchBox").on("keyup", function() {
    ...
    Table_Sample = $('#genomeTable td:visible').length;;
  });
    ...
    if (wildcardgroup !== undefined) {
      ...
      if ($("fileset").prop("checked", true)) {
        ...
        if (radButton == 'File A') {
          ...
          $(".objcectA").text("Set A: " + Table_Sample);
        } else if (radButton == 'File B') {
          addSample(group, filesetB);
          $(".objcectB").text("Set B: " + Table_Sample);
        }
  ...
  $('#clearFile').click(function() {
    ...
    $(".objcectA").text("Set A: 0");
    $(".objcectB").text("Set B: 0");
